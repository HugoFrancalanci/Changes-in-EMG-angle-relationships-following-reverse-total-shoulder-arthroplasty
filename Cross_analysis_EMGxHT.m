...
% Define the path to the SPM1D toolbox
...
% Extraction of humero-thoracic (HT) data by subject
HT_data_asymp = [];
HT_data_preop = [];
HT_data_postop = [];
...
for i = 1:length(preop_subjects)
    ...
    try
        ...
        warning('Données HT manquantes pour %s', subj);
    end
end
...
for m = 1:nb_muscles
    ...
    xlabel('Humeral elevation (°)', 'FontSize', 12, 'FontWeight', 'bold');
...
for m = 1:nb_muscles
    ...
    % RMSD entre Asymptomatic vs Postoperative
    rmsd_emg_results(m, 2) = sqrt(mean((mean_asymp - mean_postop).^2, 'omitnan'));
    
    % RMSD entre Preoperative vs Postoperative
...
fprintf('\n--- TEST 1: Inter-group variabilty (ANOVA) ---\n');
...
fprintf('\n--- TEST 2: Fit EMG-HT quality (ANOVA) ---\n');
...
if p_fit(1) < alpha
    fprintf('  Effet Groupe: p = %.4f (significant)\n', p_fit(1));
else
    fprintf('  Effet Groupe: p = %.4f (no significant)\n', p_fit(1));
end

if p_fit(2) < alpha
    fprintf('  Effet Muscle: p = %.4f (significant)\n', p_fit(2));
else
    fprintf('  Effet Muscle: p = %.4f (no significant)\n', p_fit(2));
end

fprintf('\n--- TEST 3: Inter-group variabilty by muscle ---\n');
...
fprintf('\n--- TEST 4: RMSD EMG between groups (by muscle) ---\n');
...
fprintf('\n--- TEST 5: Correlation RMSD inter-group vs fit quality ---\n');
